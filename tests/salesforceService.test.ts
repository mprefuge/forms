...
describe('SalesforceService - default RecordType behavior', () => {
  ...
  it('normalizes multipicklist values (pipes -> semicolons) when creating forms', async () => {
    ...
    expect(createMock).toHaveBeenCalled();
    const createdObj = createMock.mock.calls[0][0];
    expect(createdObj.LanguagesSpoken__c).toBe('English;American Sign Language');
  ...
  it('prefers original allowed value with spaces when provided', async () => {
    const sf = new SalesforceService({ loginUrl: 'https://login.salesforce.com', clientId: 'id', clientSecret: 'secret' });

    // Allowed values include the space variant; incoming token should be preserved as given
    jest.spyOn(sf as any, 'getRecordTypeId').mockResolvedValue('rt-general-id');
    jest.spyOn(sf as any, 'describeFormFields').mockResolvedValue([
      { name: 'ServingAreasInterest__c', type: 'picklist', picklistValues: ['United States', 'Other'] }
    ] as any);

    const createMock = jest.fn().mockResolvedValue({ success: true, id: 'form-preserve-spaces' });
    (sf as any).connection = {
      sobject: jest.fn().mockReturnValue({ create: createMock }),
      query: jest.fn().mockResolvedValue({ records: [] }),
    } as any;

    const result = await (sf as any).createForm({ ServingAreasInterest__c: 'United States' }, 'req-preserve');

    expect(createMock).toHaveBeenCalled();
    const createdObj = createMock.mock.calls[0][0];
    expect(createdObj.ServingAreasInterest__c).toBe('United States');
    expect(result.id).toBe('form-preserve-spaces');
  });
  ...
  it('falls back to a name-only campaign lookup when the record type-specific lookup does not match', async () => {
    ...
    (sf as any).connection = {
      query: jest.fn()
        ...
        .mockResolvedValueOnce({ records: [{ Id: 'camp-lookup-2', Name: 'Fall 2026 - Farmdale ESL Network Registration' }] }),
    } as any;

    const record = await (sf as any).getCampaignByNameWithFields(
      'Fall 2026 - Farmdale ESL Network Registration',
      ['Id', 'Name'],
      'Registration'
    );

    expect(record).toEqual({ Id: 'camp-lookup-2', Name: 'Fall 2026 - Farmdale ESL Network Registration' });
    expect((sf as any).connection.query).toHaveBeenNthCalledWith(
      1,
      "SELECT Id, Name FROM Campaign WHERE Name = 'Fall 2026 - Farmdale ESL Network Registration' AND RecordType.Name = 'Registration' ORDER BY CreatedDate DESC LIMIT 1"
    );
    expect((sf as any).connection.query).toHaveBeenNthCalledWith(
      2,
      "SELECT Id, Name FROM Campaign WHERE Name = 'Fall 2026 - Farmdale ESL Network Registration' ORDER BY CreatedDate DESC LIMIT 1"
    );
  });
  ...
  it('retries campaign creation without RecordTypeId when Campaign record type lookup fails', async () => {
    ...
    const result = await (sf as any).createCampaign({
      name: 'Fall 2026 - Farmdale ESL Network Registration',
      recordTypeName: 'Registration',
    });
    ...
    expect(result).toEqual({ id: 'camp-new-2', name: 'Fall 2026 - Farmdale ESL Network Registration' });
  });
